...
public partial class Result<T, TError>
{
    ...
    public Task<Result<TNew, TError>> MapAsync<TNew>(Func<T, TNew> map)
    ...
    /// <summary>
    /// Asynchronously chains this result into another result-returning synchronous operation,
    /// returning the result wrapped in a completed task.
    /// </summary>
    /// <typeparam name="TNew">The success type of the chained result.</typeparam>
    /// <param name="bind">The synchronous operation to chain.</param>
    public Task<Result<TNew, TError>> BindAsync<TNew>(Func<T, Result<TNew, TError>> bind)
        => Task.FromResult(IsSuccess
            ? bind(Value)
            : Result<TNew, TError>.Fail(Error));
    ...
    /// <summary>
    /// Asynchronously transforms the error value using a synchronous <paramref name="map"/> delegate,
    /// returning the result wrapped in a completed task.
    /// </summary>
    /// <typeparam name="TNewError">The type of the transformed error.</typeparam>
    /// <param name="map">Synchronous projection applied to the error value.</param>
    public Task<Result<T, TNewError>> MapErrorAsync<TNewError>(Func<TError, TNewError> map)
        => Task.FromResult(IsFailure
